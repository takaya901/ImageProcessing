...
output_path = '/mnt/franky2/t_ogawa/DockerTest/gray'
...
def train():
    ...
    generator_model = multi_gpu_model(load_generator(img_shape, disc_img_shape), gpus=gpu_count)
    # load discriminator model
    # discriminator_model = load_DCGAN_discriminator(img_shape, disc_img_shape, patch_num)
    discriminator_model = multi_gpu_model(load_DCGAN_discriminator(img_shape, disc_img_shape, patch_num), gpus=gpu_count)

    generator_model.compile(loss='mae', optimizer=opt_discriminator)
    discriminator_model.trainable = False

    # DCGAN_model = load_DCGAN(generator_model, discriminator_model, img_shape, patch_size)
    DCGAN_model = multi_gpu_model(load_DCGAN(generator_model, discriminator_model, img_shape, patch_size), gpus=gpu_count)    
